...
class ResCompany(models.Model):
    ...
    tools_lock_date = fields.Date(string="Lock Date", help="Can't edit documents prior to and inclusive of this "
                                                               "date.")

    @api.one
    @api.constrains('tools_lock_date')
    def _check_tools_lock_date(self):
        pick_count = self.env['tools.picking'].search_count([('date', '<=', self.tools_lock_date),
                                                             ('state', 'not in', ('done', 'cancel'))])
        if pick_count:
            raise ValidationError(_('There are %s tools pickings with same date or earlier than lock date that are '
        ...
        if self.tools_lock_date and self.period_lock_date:
            if self.tools_lock_date < self.period_lock_date:
                raise ValidationError(_('The tools lock date must be equal or greater than the lock date for '
