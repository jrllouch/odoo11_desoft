# -*- coding: utf-8 -*-
'''
    This file contains the code to print the "Highs" Report
'''
import time
...
class l10n_cu_high_report(models.AbstractModel):
    _name = 'report.l10n_cu_account_asset.l10n_cu_high_report'
    ...
    def get_report_values(self, docids, data=None):
        ...
        if asset_ids:
            ...
            if move_ids:
                self.env.cr.execute('''
                    SELECT
                        a.inventory_number, a.name, am.number, a.subscribe_date, a.initial_value, a.depreciation_tax, a.value_amount_depreciation
                    FROM
                        l10n_cu_asset_move am, l10n_cu_asset_move_category mc, account_asset_asset a, asset_move_account_asset_rel rel
                    WHERE
                        am.state = 'terminated' AND mc.id = am.asset_move_category_id AND mc.code in ('01','02','07') AND am.id = rel.asset_move_id AND rel.asset_id = a.id AND a.id in %s AND am.id in %s
                    ORDER BY
